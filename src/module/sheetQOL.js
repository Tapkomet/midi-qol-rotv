...
import { i18n, debug, debugEnabled } from "../midi-qol-rotv.js";
...
const knownSheets = {
	BetterNPCActorRotVSheet: ".item .rollable",
	ActorSheetRotVCharacter: ".item-list .item-name",
	BetterNPCActorRotVSheetDark: ".item .rollable",
	ActorSheetRotVCharacter2: ".item .item-image",
	ActorSheetRotVCharacterDark: ".item .item-image",
	DarkSheet: ".item .item-image",
	ActorNPC5EDark: ".item .item-image",
	DynamicActorSheetRotV: ".item .item-image",
	ActorSheetRotVNPC: ".item .item-image",
	ROTVBeyondCharacterSheetRotV: ".item .item-name .item-image",
	// TidyRotVSheet: ".item .item-image",
	// TidyRotVNPC: ".item .item-image",
	MonsterBlockRotV: ".item .item-name",
	"swRotV.ActorSheet5eNPC": ".item .item-name"
...
let enableSheetQOL = (app, html, data) => {
	...
	if (itemRollButtons) {
		if (["TidyRotVSheet", "TidyRotVNPC"].includes(app.constructor.name)) {
		}
		else {
	...
	<label>${i18n("midi-qol-rotv.ActorOnUseMacros")}</label>
...
function addItemSheetButtons(app, html, data, triggeringElement = "", buttonContainer = "") {
	...
	if (["BetterNPCActorRotVSheet", "BetterNPCActorRotVSheetDark"].includes(app.constructor.name)) {
...
function addItemRowButton(target, item, app, html, data, buttonContainer) {
	...
	item.getChatData().then(chatData => {
		...
		switch (item.type) {
			...
			case "power":
			case "feat":
				buttons.append(`<span class="tag"><button data-action="basicRoll">${i18n("midi-qol-rotv.buttons.roll")}</button></span>`);
				if (item.hasAttack)
					buttons.append(`<span class="tag"><button data-action="attack">${i18n("midi-qol-rotv.buttons.attack")}</button></span>`);
				if (item.hasDamage)
					buttons.append(`<span class="tag"><button data-action="damage">${i18n("midi-qol-rotv.buttons.damage")}</button></span>`);
				if (itemIsVersatile(item))
					buttons.append(`<span class="tag"><button data-action="versatileDamage">${i18n("midi-qol-rotv.buttons.versatileDamage")}</button></span>`);
				buttonsWereAdded = true;
				break;
			case "consumable":
				if (chatData.hasCharges)
					buttons.append(`<span class="tag"><button data-action="consume">${i18n("midi-qol-rotv.buttons.itemUse")} ${item.name}</button></span>`);
				buttonsWereAdded = true;
				break;
			case "tool":
				buttons.append(`<span class="tag"><button data-action="toolCheck" data-ability="${chatData.ability.value}">${i18n("midi-qol-rotv.buttons.itemUse")} ${item.name}</button></span>`);
				buttonsWereAdded = true;
				break;
		}
		buttons.append(`<span class="tag"><button data-action="info">${i18n("midi-qol-rotv.buttons.info")}</button></span>`);
		buttonsWereAdded = true;
...
function addTidyRotVItemSheetButtons(app, html, data) {
	let actor = app.object;
	$('.tidyRotV-sheet .inventory-list:not(favorites) .item').each(function () {
		let buttonContainer;
		//@ts-ignore version v10
		if (foundry.utils.isNewerVersion(game.modules.get("tidyRotV-sheet")?.version ?? "", "0.4.17"))
		...
		item.getChatData().then(chatData => {
			...
			switch (item.type) {
				...
				case "power":
				case "feat":
					buttons.append(`<a class="button" data-action="basicRoll" title="${i18n("midi-qol-rotv.buttons.roll")}"><i class="fas fa-comment-alt"></i> ${i18n("midi-qol-rotv.buttons.roll")}</a>`);
					if (item.hasAttack)
						buttons.append(`<a class="button" data-action="attack" title="Roll standard/advantage/disadvantage ${i18n("midi-qol-rotv.buttons.attack")}"><i class="fas fa-dice-d20"></i> ${i18n("midi-qol-rotv.buttons.attack")}</a>`);
					if (itemHasDamage(item))
						buttons.append(`<a class="button" data-action="damage" title="Roll ${i18n("midi-qol-rotv.buttons.damage")}"><i class="fas fa-dice-six"></i> ${i18n("midi-qol-rotv.buttons.damage")}</a>`);
					if (itemIsVersatile(item))
						buttons.append(`<a class="button" data-action="versatileDamage" title="Roll ${i18n("midi-qol-rotv.buttons.versatileDamage")}"><i class="fas fa-dice-six"></i> ${i18n("midi-qol-rotv.buttons.versatileDamage")}</a>`);
					buttonsWereAdded = true;
					break;
				case "consumable":
					if (chatData.hasCharges)
						buttons.append(`<a class="button" data-action="consume" title="${i18n("midi-qol-rotv.buttons.itemUse")} ${item.name}"><i class="fas fa-wine-bottle"></i> ${i18n("midi-qol-rotv.buttons.itemUse")} ${item.name}</a>`);
					buttonsWereAdded = true;
					break;
				case "tool":
					buttons.append(`<a class="button" data-action="toolCheck" data-ability="${chatData.ability.value}" title="${i18n("midi-qol-rotv.buttons.itemUse")} ${item.name}"><i class="fas fa-hammer"></i>  ${i18n("midi-qol-rotv.buttons.itemUse")} ${item.name}</a>`);
					buttonsWereAdded = true;
					break;
			}
			buttons.append(`<a class="button" data-action="info" title="${i18n("midi-qol-rotv.buttons.info")}"><i class="fas fa-info-circle"></i> ${i18n("midi-qol-rotv.buttons.info")}</a>`);
			buttonsWereAdded = true;
			if (buttonsWereAdded) {
				// adding the buttons to the sheet
