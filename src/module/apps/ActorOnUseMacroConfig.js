...
export class ActorOnUseMacrosConfig extends FormApplication {
	...
	static get defaultOptions() {
		return foundry.utils.mergeObject(super.defaultOptions, {
			template: "modules/midi-qol-rotv/templates/actorOnUseMacrosConfig.html",
	...
	async _updateObject(event, data) {
		await this.object.setFlag("midi-qol-rotv", "onUseMacroParts", data.onUseMacroParts);
	...
	async onMacroControl(event) {
		...
		if (a.classList.contains("delete-macro")) {
			...
			return this.object.update({ "flags.midi-qol-rotv.onUseMacroName": macros.toString() }).then(() => this.render(true));
		}
	}
}
