...
export class ItemTypeSelector extends FormApplication {
	...
	static get defaultOptions() {
		return mergeObject(super.defaultOptions, {
			...
			template: "modules/midi-qol-rotv/templates/itemTypeSelector.html",
			width: 320,
			height: "auto",
			choices: {},
			allowCustom: false,
...
async function importFromJSONDialog() {
	const content = await renderTemplate("templates/apps/import-data.html", { entity: "midi-qol-rotv", name: "settings" });
	let dialog = new Promise((resolve, reject) => {
		new Dialog({
			title: `Import midi-qol-rotv settings`,
			content: content,
			buttons: {
				import: {
...
function showDiffs(current, changed, flavor = "", title = "") {
	...
	for (let key of Object.keys(diffs)) {
		...
		let longName = i18n("midi-qol-rotv." + name + ".Name");
		if (longName.startsWith("midi-qol-rotv"))
			longName = name;
		debug("Show config changes: Name is ", name, key, key.startsWith("gm") ? "GM" : "", i18n(`midi-qol-rotv.${name + ".Name"}`));
	...
	const dialog = new Promise((resolve, reject) => {
		let dialogTitle;
		if (title !== "")
			dialogTitle = `${i18n("midi-qol-rotv.QuickSettings")} - ${title}`;
		else
			dialogTitle = i18n("midi-qol-rotv.QuickSettings");
		let d = new Dialog({
			title: dialogTitle,
...
let quickSettingsDetails = {
	...
	EnableReactions: {
		...
		codeChecks: (current, settings) => {
			...
			if (changesMade)
				ui.notifications?.warn("midi-qol-rotv Some automation enabled to support reaction processing");
		}
	},
	DisableReactions: {
	...
	NoDamageApplication: {
		...
		codeChecks: (current, settings) => {
			game.settings.set("midi-qol-rotv", "AddChatDamageButtons", "gm");
		}
	},
