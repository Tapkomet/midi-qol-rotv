import { geti18nOptions, i18n, systemConcentrationId } from "../../midi-qol-rotv.js";
...
export class TroubleShooter extends FormApplication {
	...
	constructor(object = {}, options = {}) {
		...
		this._hookId = Hooks.on("midi-qol-rotv.TroubleShooter.recordError", (errorDetail) => {
	...
	static recordError(err, message) {
		...
		Hooks.callAll("midi-qol-rotv.TroubleShooter.recordError", errorDetail);
	}
	static clearErrors() {
		this.errors = [];
	}
	...
	static get defaultOptions() {
		...
		options.title = i18n("midi-qol-rotv.TroubleShooter.Label");
		options.classes = ["midi-trouble-shooter"];
		options.id = 'midi-trouble-shooter';
		options.template = 'modules/midi-qol-rotv/templates/troubleShooter.html';
	...
	static exportTroubleShooterData() {
		...
		const filename = "fvtt-midi-qol-rotv-troubleshooter.json";
	...
	async close(...args) {
		Hooks.off("midi-qol-rotv.TroubleShooter.recordError", this._hookId);
		super.close(...args);
	}
	activateListeners(html) {
		html.find("#midi-qol-rotv-export-troubleshooter").on("click", (event) => {
			event.preventDefault();
			event.stopPropagation();
			TroubleShooter.exportTroubleShooterData();
		});
		html.find("#midi-qol-rotv-import-troubleshooter").on("click", async (event) => {
			event.preventDefault();
			event.stopPropagation();
		...
		html.find("#midi-qol-rotv-regenerate-troubleshooter").on("click", (event) => {
			event.preventDefault();
			event.stopPropagation();
			TroubleShooter.data = TroubleShooter.collectTroubleShooterData();
			this.render(true);
		});
		html.find("#midi-qol-rotv-clear-errors-troubleshooter").on("click", (event) => {
			event.preventDefault();
			event.stopPropagation();
		...
		html.find("#midi-qol-rotv-overwrite-midi-settings").on("click", (event) => {
			event.preventDefault();
			event.stopPropagation();
	...
	async overWriteMidiSettings() {
		...
		if (TroubleShooter.data.isLocal) {
			ui.notifications?.warn(`midi-qol-rotv | Cant set midi settings - you have not loaded external trouble shooter data`);
			return false;
		}
		let dialog = new Promise((resolve, reject) => {
			new Dialog({
				title: `Oeverwrite midi-qol-rotv settings from loaded file`,
	...
	static collectTroubleShooterData() {
		...
		data.summary = {
			...
			//@ts-expect-error .version
			"midi-qol-rotv-version": game.modules.get("midi-qol-rotv")?.version,
			//@ts-expect-error .version
		...
		if (game.modules.get("ddb-importer")?.active) {
			...
			data.summary.moduleSettings["DDB Importer"] = i18n("midi-qol-rotv.Inactive");
		if (game.modules.get("dfreds-convenient-effects")?.active) {
			...
			data.summary.moduleSettings["Convenient Effects"] = i18n("midi-qol-rotv.Inactive");
		if (game.modules.get("monks-little-details")?.active) {
			...
			data.summary.moduleSettings["Monk's Little Details"] = i18n("midi-qol-rotv.Inactive");
		if (game.modules.get("monks-tokenbar")?.active) {
			...
			data.summary.moduleSettings["Monks Token Bar"] = i18n("midi-qol-rotv.Inactive");
		if (game.modules.get("sequencer")?.active) {
			...
			data.summary.moduleSettings["Sequencer"] = i18n("midi-qol-rotv.Inactive");
		if (game.modules.get("times-up")?.active) {
			...
			data.summary.moduleSettings["Times-Up"] = i18n("midi-qol-rotv.Inactive");
		if (game.modules.get("tokenmagic")?.active) {
			...
			data.summary.moduleSettings["Token Magic FX"] = i18n("midi-qol-rotv.Inactive");
		...
		data.summary.midiSettings["ROTV V3 Damage Application"] = configSettings.v3DamageApplication;
		...
		*/
		for (let moduleData of game.modules) {
			...
			switch (module.id) {
				...
				case "attack-roll-check-RotV":
				case "betterrollsRotV":
				case "dice-rng-protector":
				case "effective-transferral":
				case "fast-rolls":
				case "faster-rolling-by-default-RotV":
				case "gm-paranoia-taragnor":
				case "max-crit":
				case "mre-rotv":
				case "multiattack-RotV":
				case "obsidian":
				case "quick-rolls":
				case "ready-set-roll-RotV":
				case "roll-tooltips-RotV":
				case "retroactive-advantage-RotV":
				case "rollgroups":
				case "wire":
		...
		data.summary.outOfDate = Object.keys(data.modules)
			...
			.map(key => {
			const versionString = `${data.modules[key].active ? i18n("midi-qol-rotv.Active") : i18n("midi-qol-rotv.Inactive")} ${data.modules[key].version}`;
			return {
				key,
			...
			if (walledTemplatesTargeting && midiTargeting) {
				data.problems.push({
					...
					/*          fixerFunc: async function (app: TroubleShooter) {
								if (!game.user?.isGM) {
								ui.notifications?.error("midi-qol-rotv | You must be a GM to fix walled templates auto target");
								return;
								}
								...
								await game.settings.set("midi-qol-rotv", "ConfigSettings", configSettings);
				...
				data.problems.push({
					...
					fixerFunc: async function (app) {
						if (!game.user?.isGM) {
							ui.notifications?.error("midi-qol-rotv | You must be a GM to fix walled templates settings");
							return;
						}
	...
	static checkItemMacro(data) {
		...
		if (safeGetGameSetting('itemacro', 'charsheet')) {
			data.problems.push({
				...
				fixerFunc: async function (app) {
					if (!game.user?.isGM) {
						ui.notifications?.error("midi-qol-rotv | You must be a GM to fix Item Macro char sheet flag");
						return;
					}
	...
	static checkMidiSettings(data) {
		if (!(safeGetGameSetting("midi-qol-rotv", "EnableWorkflow"))) {
			data.problems.push({
				moduleId: "midi-qol-rotv",
				severity: "Warn",
				...
				fixerFunc: async function (app) {
					game.settings.set("midi-qol-rotv", "EnableWorkflow", true).then(() => {
						fetchParams();
						TroubleShooter.data = TroubleShooter.collectTroubleShooterData();
						app.render(true);
					});
