...
export class SoundConfigPanel extends FormApplication {
	...
	static get defaultOptions() {
		return foundry.utils.mergeObject(super.defaultOptions, {
			id: "midi-qol-rotv-sound-config",
			tabs: [{ navSelector: ".tabs", contentSelector: ".content", initial: "sounds" }],
			classes: ["rotv"],
			title: "Sound Config",
			template: "modules/midi-qol-rotv/templates/sound-config.html",
			height: "auto",
			choices: {},
			allowCustom: false,
			width: "auto",
	...
	getData(options) {
		...
		//@ts-ignore
		let systemConfig = (game.system.id === "rotv") ? CONFIG.ROTV : CONFIG.SW5E;
	...
	async _updateObject(event, formData) {
		...
		await game.settings.set("midi-qol-rotv", "MidiSoundSettings", settings);
	}
