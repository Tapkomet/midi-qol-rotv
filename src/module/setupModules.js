import { error, debugEnabled } from "../midi-qol-rotv.js";
...
export function getModuleVersion(moduleName) {
	...
	if (!/[0-9\.]+/.test(modVer)) {
		console.warn(`midi-qol-rotv | module ${moduleName} has unrecognised version ${modVer} using ${REQUIRED_MODULE_VERSIONS[moduleName]} instead}`);
...
export let setupModules = () => {
	...
	if (foundry.utils.isNewerVersion(game.system.version, "2.4.99")) {
		ui.notifications?.error("Midi qol has not been tested with rotv 2.5.0 and there are known breaking changes");
	}
	for (let name of Object.keys(REQUIRED_MODULE_VERSIONS)) {
		...
		if (!installedModules.get(name)) {
			if (game.modules.get(name)?.active) {
				...
				const message = `midi-qol-rotv requires ${name} to be of version ${REQUIRED_MODULE_VERSIONS[name]} or later, but it is version ${game.modules.get(name)?.version}`;
				error(message);
				TroubleShooter.recordError(new Error(message), message);
			}
			else
				console.warn(`midi-qol-rotv | optional module ${name} not active - some features disabled`);
		}
	}
	if (debugEnabled > 0) {
...
export function checkModules() {
	if (game.user?.isGM && !installedModules.get("socketlib")) {
		ui.notifications?.error("midi-qol-rotv.NoSocketLib", { permanent: true, localize: true });
	}
	//@ts-ignore
	const midiVersion = game.modules.get("midi-qol-rotv").version;
	const notificationVersion = game.settings.get("midi-qol-rotv", "notificationVersion");
}
