import { error, debugEnabled, i18n } from "../midi-qol-rotv.js";
	...
	for (let name of Object.keys(modules)) {
		...
		if (!installedModules.get(name)) {
			if (game.modules.get(name)?.active) {
				...
				error(`midi-qol-rotv requires ${name} to be of version ${modules[name]} or later, but it is version ${game.modules.get(name)?.version}`);
			}
			else
				console.warn(`midi-qol-rotv | module ${name} not active - some features disabled`);
		}
	}
	if (debugEnabled > 0) {
...
export function checkCubInstalled() {
	...
	if (game.user?.isGM && configSettings.concentrationAutomation && !installedModules.get("combat-utility-belt")) {
		let d = new Dialog({
			// localize this text
			title: i18n("midi-qol-rotv.confirm"),
			content: i18n("midi-qol-rotv.NoCubInstalled"),
			buttons: {
				one: {
