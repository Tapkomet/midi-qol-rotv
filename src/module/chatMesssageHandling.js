import { debug, warn, i18n, error, gameStats, debugEnabled, MQdefaultDamageType, i18nFormat } from "../midi-qol-rotv.js";
...
export function betterRollsUpdate(message, update, options, user) {
	...
	if (update.flags && update.flags["midi-qol-rotv"]) {
...
export let processCreateBetterRollsMessage = (message, user) => {
	...
	//@ts-ignore
	if (debugEnabled > 1)
		debug("process preCreateBetterRollsCard ", message, installedModules["betterrolls5e"], message.content?.startsWith('<div class="rotv red-full chat-card"'));
	let actorId = brFlags.actorId;
	let tokenId = brFlags.tokenId;
	if (tokenId && !tokenId.startsWith("Scene")) { // remove when BR passes a uuid instead of constructed id.
		const parts = tokenId.split(".");
	...
	const storedData = message.getFlag("rotv", "itemData") ?? brFlags.params.itemData;
	...
	if (hasEffects && !configSettings.autoItemEffects) {
		...
		const button = `<button data-action="applyEffects">${i18n("midi-qol-rotv.ApplyEffects")}</button>`;
...
export let diceSoNiceHandler = async (message, html, data) => {
	...
	if (!getProperty(message, "flags.midi-qol-rotv.waitForDiceSoNice"))
		return;
	if (debugEnabled > 1)
	...
	Hooks.once("diceSoNiceRollComplete", (id) => {
		let savesDisplay = $(html).find(".midi-qol-rotv-saves-display").length === 1;
		let hitsDisplay = configSettings.mergeCard ?
			$(html).find(".midi-qol-rotv-hits-display").length === 1
			: $(html).find(".midi-qol-rotv-single-hit-card").length === 1;
		if (savesDisplay) {
...
export function checkOverTimeSaves(message, data, options, user) {
	if (!message.rolls?.length || !["skill", "save", "check"].includes(data.flags?.rotv?.roll?.type))
		return true;
	...
	try {
		...
		func(actor, data.flags.rotv.roll, message.rolls[0]);
	}
	finally {
		return true;
	}
...
export let hideStuffHandler = (message, html, data) => {
	...
	if (getProperty(message, "flags.monks-tokenbar"))
		return;
	const midiqolFlags = getProperty(message, "flags.midi-qol-rotv");
	...
	if (game.user?.isGM) {
		let ids = html.find(".midi-qol-rotv-target-name");
		...
		&& message.isRoll
		&& (message.flavor?.includes(i18n("ROTV.ActionSave")) || message.flavor?.includes(i18n("ROTV.ActionAbil")))
		) {
		html.hide();
		}
	...
	*/
		...
		if (configSettings.autoCheckSaves === "allNoRoll")
			html.find(".midi-qol-rotv-save-total").hide();
		// Hide the save dc if required
		if (!configSettings.displaySaveDC) {
			html.find(".midi-qol-rotv-saveDC").hide();
		}
		if (message.blind) {
			// html.find(".midi-qol-rotv-attack-roll .dice-total").text(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`);
			html.find(".midi-qol-rotv-attack-roll .dice-roll").replaceWith(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`);
			html.find(".midi-qol-rotv-damage-roll").find(".dice-roll").replaceWith(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`);
			html.find(".midi-qol-rotv-other-roll").find(".dice-roll").replaceWith(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`);
			html.find(".midi-qol-rotv-bonus-roll").find(".dice-roll").replaceWith(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`);
		...
		html.find(".midi-qol-rotv-target-npc-GM").hide();
		if (message.user?.isGM) {
			const d20AttackRoll = getProperty(message.flags, "midi-qol-rotv.d20AttackRoll");
			if (configSettings.hideRollDetails === "all" || getProperty(message.flags, "midi-qol-rotv.GMOnlyAttackRoll")) {
				...
				if (d20AttackRoll && configSettings.hideRollDetails === "d20AttackOnly") {
					html.find(".midi-qol-rotv-attack-roll .dice-total").text(`(d20) ${d20AttackRoll}`);
					html.find(".midi-qol-rotv-damage-roll").find(".dice-roll").replaceWith(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`);
					html.find(".midi-qol-rotv-other-roll").find(".dice-roll").replaceWith(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`);
					html.find(".midi-qol-rotv-bonus-roll").find(".dice-roll").replaceWith(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`);
				}
					...
					html.find(".midi-qol-rotv-attack-roll .dice-total").text(`(d20) ${d20AttackRoll}`);
					html.find(".midi-qol-rotv-other-roll").find(".dice-tooltip").remove();
					html.find(".midi-qol-rotv-other-roll").find(".dice-formula").remove();
					html.find(".midi-qol-rotv-bonus-roll").find(".dice-tooltip").remove();
					html.find(".midi-qol-rotv-bonus-roll").find(".dice-formula").remove();
					...
					*/
					...
					const hitFlag = getProperty(message.flags, "midi-qol-rotv.isHit");
					const hitString = hitFlag === undefined ? "" : hitFlag ? i18n("midi-qol-rotv.hits") : i18n("midi-qol-rotv.misses");
					html.find(".midi-qol-rotv-attack-roll .dice-total").text(`${hitString}`);
...
export let chatDamageButtons = (message, html, data) => {
	...
	if (["other", "damage"].includes(message.flags?.rotv?.roll?.type)) {
		let item;
		let itemId;
		let actorId = message.speaker.actor;
		if (message.flags?.rotv?.roll?.type === "damage") {
	...
	else if (getProperty(message, "flags.midi-qol-rotv.damageDetail") || getProperty(message, "flags.midi-qol-rotv.otherDamageDetail")) {
		let midiFlags = getProperty(message, "flags.midi-qol-rotv");
		addChatDamageButtonsToHTML(midiFlags.damageTotal, midiFlags.damageDetail, html, midiFlags.actorUuid, midiFlags.itemUuid, "damage", ".midi-qol-rotv-damage-roll .dice-total");
		addChatDamageButtonsToHTML(midiFlags.otherDamageTotal, midiFlags.otherDamageDetail, html, midiFlags.actorUuid, midiFlags.itemUuid, "other", ".midi-qol-rotv-other-roll .dice-total");
		addChatDamageButtonsToHTML(midiFlags.bonusDamageTotal, midiFlags.bonusDamageDetail, html, midiFlags.actorUuid, midiFlags.itemUuid, "bonus", ".midi-qol-rotv-bonus-roll .dice-total");
	}
	return true;
};
...
export function processItemCardCreation(message, user) {
	if (game.settings.get("midi-qol-rotv", "itemUseHooks"))
		return;
	const midiFlags = message.flags["midi-qol-rotv"];
...
export async function onChatCardAction(event) {
	...
	if (betterRollsFlags) {
		...
		if (!item) { // TODO investigate why this is occurring
			// return ui.notifications.error(game.i18n.format("ROTV.ActionWarningNoItem", {item: card.dataset.itemId, name: actor.name}))
		}
	}
	if (!actor || !item)
		return;
