import { debug, debugEnabled, error, log, warn } from "../midi-qol-rotv.js";
...
export function updateUndoChatCardUuidsById(data) {
	...
	if (!currentUndo) {
		console.warn("midi-qol-rotv | updateUndoChatCardUuidsById | Could not find existing entry for ", data);
		return;
	}
...
export function updateUndoChatCardUuids(data) {
	...
	if (!currentUndo) {
		console.warn("midi-qol-rotv | updateUndoChatCardUuids | Could not find existing entry for ", data);
		return;
	}
	currentUndo.chatCardUuids = data.chatCardUuids;
}
...
export function queueUndoData(data) {
	...
	data.targets.forEach(undoEntry => {
		...
		/* This replaces the use of midi-qol-rotv concentration data but I'm not sure it's needed
		//@ts-expect-error
...
export function addQueueEntry(queue, data) {
	...
	if (!added)
		queue.push(data);
	Hooks.callAll("midi-qol-rotv.addUndoEntry", data);
...
export async function undoTillWorkflow(workflowId, undoTarget, removeWorkflow = false) {
	...
	try {
		...
		if (queueLength !== undoDataQueue.length)
			Hooks.callAll("midi-qol-rotv.removeUndoEntry");
	}
...
export async function _undoMostRecentWorkflow() {
	...
	try {
		...
		if (undoData)
			Hooks.callAll("midi-qol-rotv.removeUndoEntry", undoData);
	}
	return;
}
