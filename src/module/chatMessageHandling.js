import { debug, warn, i18n, error, debugEnabled, i18nFormat, MODULE_ID } from "../midi-qol-rotv.js";
...
export function checkOverTimeSaves(message, data, options, user) {
	if (!message.rolls?.length || !["skill", "save", "ability"].includes(data.flags?.rotv?.roll?.type))
		return true;
	...
	if (!actor)
		return true;
	const overtimeActorUuid = foundry.utils.getProperty(message, "flags.midi-qol-rotv.overtimeActorUuid");
	...
	try {
		let func = async (actor, rollFlags, roll) => {
			...
			for (let effect of actor.effects.filter(ef => ef.changes.some(change => change.key === "flags.midi-qol-rotv.OverTime"))) {
				await doOverTimeEffect(actor, effect, true, { saveToUse: roll, rollFlags: data.flags?.rotv?.roll, isActionSave: true });
			}
		};
		func(actor, data.flags.rotv.roll, message.rolls[message.rolls.length - 1]);
	}
	catch (err) {
...
export let hideStuffHandler = (message, html, data) => {
	...
	const midiqolFlags = foundry.utils.getProperty(message, "flags.midi-qol-rotv");
	...
	if (game.user?.id !== message.user?.id) {
		html.find(".midi-qol-rotv-attack-buttons").hide();
		html.find(".midi-qol-rotv-damage-buttons").hide();
		html.find(".midi-qol-rotv-otherDamage-button").hide();
		html.find(".midi-qol-rotv-versatile-damage-button").hide();
	}
	let ids = html.find(".midi-qol-rotv-target-select");
	ids.hover(_onTargetHover, _onTargetHoverOut);
	ids.click(_onTargetSelect);
	ids.contextmenu(_onTargetShow);
	if (game.user?.isGM) {
		html.find(".midi-qol-rotv-playerTokenName").remove();
		if (configSettings.hidePlayerDamageCard && $(html).find(".midi-qol-rotv-player-damage-card").length)
			html.hide();
		if ($(html).find(".midi-qol-rotv-hits-display").length) {
			if (configSettings.mergeCard) {
				$(html).find(".midi-qol-rotv-hits-display").show();
			}
			else {
				html.show();
			}
		}
		html.find(".midi-qol-rotv-target-npc-Player").hide();
		//@ts-expect-error
		...
		html.find(".midi-qol-rotv-save-tooltip").hide();
		if (message.blind) {
			html.find(".midi-attack-roll .dice-roll").replaceWith(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`);
			// html.find(".midi-damage-roll .dice-roll").replaceWith(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`);
			if (!(message.flags && message.flags["monks-tokenbar"])) // not a monks roll
				html.find(".dice-roll").replaceWith(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`);
			// html.find(".dice-result").replaceWith(`<span>${i18n("midi-qol-rotv.DiceRolled")}</span>`); Monks saving throw css
			//TODO this should probably just check formula
		}
		if (configSettings.autoCheckHit === "whisper" || message.blind || safeGetGameSetting("rotv", "attackRollVisibility") === "none") {
			if (configSettings.mergeCard) {
				html.find(".midi-qol-rotv-hits-display").hide();
			}
			else if (html.find(".midi-qol-rotv-single-hit-card").length === 1 && data.whisper) {
				html.hide();
			}
		}
		if ((configSettings.autoCheckSaves === "whisper" || message.blind)) {
			if (configSettings.mergeCard) {
				html.find(".midi-qol-rotv-saves-display").hide();
			}
			else if (html.find(".midi-qol-rotv-saves-display").length === 1 && data.whisper) {
				html.hide();
			}
		}
		...
		if (game.user?.isGM)
			...
			switch (safeGetGameSetting("rotv", "challengeVisibility")) {
				case "all":
					shouldDisplayChallenge = true;
					break;
				case "player":
		...
		if (!configSettings.displaySaveDC || !shouldDisplayChallenge) {
			html.find(".midi-qol-rotv-saveDC").remove();
			if (!["allShow", "all"].includes(configSettings.autoCheckSaves)) {
				html.find(".midi-qol-rotv-npc-save-total").remove();
			}
		}
		if (!shouldDisplayChallenge) {
			html.find(".midi-qol-rotv-saves-display .midi-qol-rotv-save-symbol").remove();
			html.find(".midi-qol-rotv-saves-display .midi-qol-rotv-save-class").removeClass("success");
			html.find(".midi-qol-rotv-saves-display .midi-qol-rotv-save-class").removeClass("failure");
			html.find(".midi-qol-rotv-saves-display .midi-qol-rotv-save-class").removeClass("critical");
			html.find(".midi-qol-rotv-saves-display .midi-qol-rotv-save-class").removeClass("fumble");
		}
		if (safeGetGameSetting("rotv", "attackRollVisibility")) {
			const visibility = safeGetGameSetting("rotv", "attackRollVisibility");
			if (visibility === "none") {
				html.find(".midi-qol-rotv-attack-roll .dice-total").removeClass("success");
				html.find(".midi-qol-rotv-attack-roll .dice-total").removeClass("failure");
				html.find(".midi-qol-rotv-attack-roll .dice-total").removeClass("fumble");
				html.find(".midi-qol-rotv-attack-roll .dice-total").removeClass("critical");
				html.find(".midi-qol-rotv-hits-display .midi-qol-rotv-hit-symbol").remove();
				html.find(".midi-qol-rotv-hits-display .midi-qol-rotv-hit-class").removeClass("success");
				html.find(".midi-qol-rotv-hits-display .midi-qol-rotv-hit-class").removeClass("failure");
				html.find(".midi-qol-rotv-hit-symbol").remove();
				html.find(".midi-qol-rotv-npc-ac").remove();
			}
		...
		if (message.user?.id !== game.user?.id || configSettings.confirmAttackDamage === "gmOnly") {
			html.find(".midi-qol-rotv-confirm-damage-roll-complete-hit").hide();
			html.find(".midi-qol-rotv-confirm-damage-roll-complete-miss").hide();
			html.find(".midi-qol-rotv-confirm-damage-roll-complete-critical").hide();
		}
		if (!game.user?.isGM) {
			// Can update the attack roll here, but damage rolls are redone in the ChatmessageMidi code so do the hiding for those there
			html.find(".midi-qol-rotv-confirm-damage-roll-cancel").hide();
			// hide the gm version of the name from` players
			html.find(".midi-qol-rotv-gmTokenName").remove();
		}
	}
	//@ts-expect-error
...
export let chatDamageButtons = (message, html, data) => {
	if (debugEnabled > 1)
		debug("Chat Damage Buttons ", addChatDamageButtons, message, message.flags?.rotv?.roll?.type, message.flags);
	...
	if (["damage", "other"].includes(message.flags?.rotv?.roll?.type)) {
		let item;
		let itemId;
		let actorId = message.speaker.actor;
		//@ts-expect-error
		let theRolls = message.rolls.filter(r => r instanceof CONFIG.Dice.DamageRoll || message.flags.rotv.roll.type === "other");
		if (theRolls.length === 0)
			return;
		if (["damage", "other"].includes(message.flags?.rotv?.roll?.type)) {
			itemId = message.flags.rotv?.roll.itemId;
	...
	else if (foundry.utils.getProperty(message, "flags.midi-qol-rotv.damageDetail") || foundry.utils.getProperty(message, "flags.midi-qol-rotv.otherDamageDetail")) {
		let midiFlags = foundry.utils.getProperty(message, "flags.midi-qol-rotv");
		let targetField = ".dice-formula";
		if (["formula", "formulaadv"].includes(configSettings.rollAlternate))
			targetField = ".dice-formula";
		addChatDamageButtonsToHTML(midiFlags.damageTotal, midiFlags.damageDetail, html, midiFlags.actorUuid, midiFlags.itemUuid, "damage", `.midi-qol-rotv-damage-roll ${targetField}`);
		addChatDamageButtonsToHTML(midiFlags.otherDamageTotal, midiFlags.otherDamageDetail, html, midiFlags.actorUuid, midiFlags.itemUuid, "other", `.midi-qol-rotv-other-damage-roll ${targetField}`);
		addChatDamageButtonsToHTML(midiFlags.bonusDamageTotal, midiFlags.bonusDamageDetail, html, midiFlags.actorUuid, midiFlags.itemUuid, "bonus", `.midi-qol-rotv-bonus-damage-roll ${targetField}`);
	}
	return true;
};
export function addChatDamageButtonsToHTML(totalDamage, damageList, html, actorId, itemUuid, tag = "damage", toMatch = ".dice-total", style = "margin: 0px;") {
	...
	let btnStylingLimeGreen = `background-color:var(--rotv-color-success); ${style}`;
	let btnStylingLightGreen = `background-color:var(--rotv-color-success-background); ${style}`;
	let btnStylingRed = `background-color: var(--rotv-color-failure-background); ${style}`;
		...
		case "applyEffects":
			...
			if (game.user?.id !== message.user?.id) {
				...
				if (game.user.targets.size === 0) {
					ui.notifications?.warn(i18n("midi-qol-rotv.noTokens"));
					return;
				}
				...
				if (workflow) {
					...
					ui.notifications?.warn(i18nFormat("midi-qol-rotv.NoWorkflow", { itemName: item.name }));
				}
			}
			break;
...
export function processCreateDDBGLMessages(message, options, user) {
	...
	if (!(["attack", "damage", "heal"].includes(flags.rotv?.roll?.type)))
		return;
	const itemId = flags.rotv?.roll?.itemId;
	if (!itemId) {
	...
	if (!item) {
		error(`ddb-game-log roll could not find item ${flags.rotv.roll.itemId} on actor ${actor.name}`);
		return;
	}
	let workflow = DDBGameLogWorkflow.get(item.uuid);
	if (!workflow && flags.rotv.roll.type === "damage" && item.hasAttack && ["rwak", "mwak"].includes(item.actionType)) {
	...
	if (flags.rotv.roll.type === "attack") {
		//@ts-expect-error
	...
	if (["damage", "heal"].includes(flags.rotv.roll.type)) {
		//@ts-expect-error
		let rolls = message.rolls;
		if (!rolls)
			return;
