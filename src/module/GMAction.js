...
import { i18n, log, warn, gameStats, getCanvas, error, debugEnabled, debugCallTiming } from "../midi-qol-rotv.js";
...
export let setupSocket = () => {
	socketlibSocket = globalThis.socketlib.registerModule("midi-qol-rotv");
...
async function prepareDamageListItems(data, templateData, tokenIdList, createPromises = false, showNPC = true) {
	...
	for (let damageItem of damageList) {
		...
		const DRFlags = actorFlags["midi-qol-rotv"] ? actorFlags["midi-qol-rotv"].DR : undefined;
		if (DRFlags) {
			listItem["DR"] = "DR: ";
...
async function createPlayerDamageCard(data) {
	...
	let tokenIdList = [];
	let templateData = {
		damageApplied: ["yes", "yesCard"].includes(data.autoApplyDamage) ? i18n("midi-qol-rotv.HPUpdated") : i18n("midi-qol-rotv.HPNotUpdated"),
		damageList: [],
...
async function createGMReverseDamageCard(data) {
	...
	let templateData = {
		damageApplied: (["yes", "yesCard"].includes(data.autoApplyDamage) || data.forceApply) ?
			i18n("midi-qol-rotv.HPUpdated") :
			data.autoApplyDamage === "yesCardNPC" ? i18n("midi-qol-rotv.HPNPCUpdated") : i18n("midi-qol-rotv.HPNotUpdated"),
		damageList: [],
		needsButtonAll: false
	};
