...
import { i18n, log, warn, gameStats, getCanvas, error, debugEnabled, debugCallTiming, debug, GameSystemConfig, MODULE_ID } from "../midi-qol-rotv.js";
		...
		switch (handler) {
			...
			case "_gmUnsetFlag":
				if (game.user?.isTrusted)
					return true;
				ui.notifications?.warn(`midi-qol-rotv | user ${game.user?.name} must be a trusted player to call ${handler} and will be disabled in the future`);
...
async function confirmDamageRollComplete(data) {
	...
	if ((workflow.currentAction === workflow.WorkflowState_AttackRollComplete) || hasHits &&
		...
		return "midi-qol-rotv | You must roll damage before completing the roll - you can only confirm miss until then";
	}
...
async function confirmDamageRollCompleteHit(data) {
	...
	for (let hitDataKey in workflow.hitDisplayData) {
		workflow.hitDisplayData[hitDataKey].hitString = i18n("midi-qol-rotv.hits");
...
async function confirmDamageRollCompleteMiss(data) {
	...
	if (workflow.hitTargets.size > 0 || workflow.hitTargetsEC.size > 0) {
		...
		const rollMode = game.settings.get("core", "rollMode");
		for (let hitDataKey in workflow.hitDisplayData) {
			workflow.hitDisplayData[hitDataKey].hitString = i18n("midi-qol-rotv.misses");
			if (configSettings.highlightSuccess) {
...
async function updateActor(data) {
	...
	if (!actor)
		return;
	if (data.actorData) {
		console.warn(`midi-qol-rotv | updateActor actorData deprecated. Call await MidiQOL.socket().execuateAsGM("updateActor"({ updates }) instead`);
...
async function addConvenientEffect(options) {
	...
	console.warn("midi-qol-rotv | Deprecated. Call await game.dfreds.effectInterface?.addEffect({ effectName, uuid: actorUuid, origin }) instead");
	//@ts-ignore
...
export function initGMActionSetup() {
	traitList.di = i18n("ROTV.DamImm");
	traitList.dr = i18n("ROTV.DamRes");
	traitList.dv = i18n("ROTV.DamVuln");
	traitList.da = "da";
	traitList.dm = "dm";
	traitList.di = "di";
	traitList.dr = "dr";
	...
	return new Promise(async (resolve) => {
		...
		if (configSettings.playerSaveTimeout > 0)
			timeoutId = setTimeout(async () => {
				...
				foundry.utils.setProperty(roll, "flags.midi-qol-rotv.rollType", data.options?.midiType);
				resolve(result ?? {});
		...
		foundry.utils.setProperty(result, "flags.midi-qol-rotv.rollType", data.options?.midiType);
		resolve(result ?? {});
	});
}
		...
		if (createPromises && doHits && (["yes", "yesCard", "yesCardNPC", "yesCardMisses"].includes(data.autoApplyDamage) || data.forceApply)) {
			if ((hpDamage || tempDamage || newVitality !== oldVitality) && (data.autoApplyDamage !== "yesCardNPC" || actor.type !== "character")) {
				...
				if (actor.isOwner) {
					...
					if (Hooks.call("rotv.preApplyDamage", actor, hpDamage /*+ tempDamage */ + (vitalityDamage ?? 0), updates, updateContext))
		...
		if (configSettings.v3DamageApplication) {
			...
			["di", "dv", "dr"].forEach(trait => {
				...
				if (traits.value instanceof Array && (traits?.custom || traits?.value.length > 0)) {
					//@ts-expect-error CONFIG.ROTV
					listItem[trait] = (`${traitList[trait]}: ${traits.value.map(t => CONFIG.ROTV.damageResistanceTypes[t]).join(",").concat(" " + traits?.custom)}`);
				}
...
async function createPlayerDamageCard(data) {
	...
	let tokenIdList = [];
	let templateData = {
		damageApplied: ["yes", "yesCard", "yesCardMisses"].includes(data.autoApplyDamage) ? i18n("midi-qol-rotv.HPUpdated") : i18n("midi-qol-rotv.HPNotUpdated"),
		damageList: [],
	...
	if (["yesCard", "noCard", "yesCardNPC", "yesCardMisses", "noCardMisses"].includes(data.autoApplyDamage)) {
		const content = await renderTemplate("modules/midi-qol-rotv/templates/damage-results-player.html", templateData);
...
async function createGMReverseDamageCard(data, doHits = true) {
	...
	let templateData = {
		damageWasApplied,
		damageApplied: damageWasApplied ? i18n("midi-qol-rotv.HPUpdated") : data.autoApplyDamage === "yesCardNPC" ? i18n("midi-qol-rotv.HPNPCUpdated") : i18n("midi-qol-rotv.HPNotUpdated"),
		damageList: [],
		needsButtonAll: false
	};
	...
	if (["yesCard", "noCard", "yesCardNPC", "yesCardMisses", "noCardMisses"].includes(data.autoApplyDamage)) {
		const content = await renderTemplate("modules/midi-qol-rotv/templates/damage-results.html", templateData);
		const speaker = ChatMessage.getSpeaker();
...
export let processUndoDamageCard = (message, html, data) => {
	...
	button.click((ev) => {
		(async () => {
			for (let { actorUuid, oldTempHP, oldHP, totalDamage, newHP, newTempHP, oldVitality, newVitality, damageItem } of message.flags.midiqol.undoDamage) {
				...
				applyButton.children()[0].classList.add("midi-qol-rotv-enable-damage-button");
				applyButton.children()[0].classList.remove("midi-qol-rotv-disable-damage-button");
				const reverseButton = html.find(`#reverse-${actorUuid.replaceAll(".", "")}`);
				reverseButton.children()[0].classList.remove("midi-qol-rotv-enable-damage-button");
				reverseButton.children()[0].classList.add("midi-qol-rotv-disable-damage-button");
				let actor = fromActorUuid(actorUuid);
	...
	button.click((ev) => {
		(async () => {
			for (let { actorUuid, oldTempHP, oldHP, totalDamage, newHP, newTempHP, damageItem, oldVitality, newVitality } of message.flags.midiqol.undoDamage) {
				...
				let actor = fromActorUuid(actorUuid);
				const applyButton = html.find(`#apply-${actorUuid.replaceAll(".", "")}`);
				applyButton.children()[0].classList.add("midi-qol-rotv-disable-damage-button");
				applyButton.children()[0].classList.remove("midi-qol-rotv-enable-damage-button");
				const reverseButton = html.find(`#reverse-${actorUuid.replaceAll(".", "")}`);
				reverseButton.children()[0].classList.remove("midi-qol-rotv-disable-damage-button");
				reverseButton.children()[0].classList.add("midi-qol-rotv-enable-damage-button");
				log(`Setting HP to ${newTempHP} and ${newHP}`);
				const update = { "system.attributes.hp.temp": newTempHP, "system.attributes.hp.value": newHP };
	...
	message.flags.midiqol.undoDamage.forEach(({ actorUuid, oldTempHP, oldHP, totalDamage, newHP, newTempHP, oldVitality, newVitality, damageItem }) => {
		...
		button.click((ev) => {
			ev.currentTarget.children[0].classList.add("midi-qol-rotv-disable-damage-button");
			ev.currentTarget.children[0].classList.remove("midi-qol-rotv-enable-damage-button");
			const otherButton = html.find(`#apply-${actorUuid.replaceAll(".", "")}`);
			otherButton.children()[0].classList.remove("midi-qol-rotv-disable-damage-button");
			otherButton.children()[0].classList.add("midi-qol-rotv-enable-damage-button");
			(async () => {
				let actor = fromActorUuid(actorUuid);
